...
import axios from 'axios' // help to send HTTP request

createConnection().then(db => {
    const productRepository = db.getMongoRepository(Product)
    amqp.connect('amqps://lddsdplh:27qOSThAbx70ZPc4s4m18HwESLGWXzjz@armadillo.rmq.cloudamqp.com/lddsdplh', (error0, connection) => {
        if (error0) {
            throw error0
        }
